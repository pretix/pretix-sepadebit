    ...
    def __call__(self, value):
        ...
        if any(
            ...
            raise ValidationError(
                _(
                    "Direct debit is not allowed for this IBAN, please get in touch with the event organizer or "
            ...
            for k in range(6, 15):
                if iban_without_checksum[:k] in DATA:
                    ...
                    if len(input_bic) < len(correct_bic):
                        input_bic += "XXX"
                    bic_match = correct_bic == input_bic or (
                        correct_bic.startswith("COBADE")
                        and input_bic.startswith("COBADE")
                    )  # https://github.com/pretix/pretix-sepadebit/issues/34
                    if not bic_match:
                        raise ValidationError(
                            _(
                                "The BIC number {bic} does not match the IBAN. Please double, check your banking "
                                "details. According to our data, the correct BIC would be {correctbic}."
                            ).format(bic=input_bic, correctbic=correct_bic)
                        )
...
class SepaDebit(BasePaymentProvider):
        ...
        d = OrderedDict(
            [
                (
                    ...
                    forms.BooleanField(
                        label=_(
                            "I have understood that I need to regularly create SEPA XML export files and transfer "
                ...
                (
                    ...
                    forms.CharField(
                        ...
                        required=False,
                        help_text=_(
                            "We will use this string and append the event slug and the order code to build a "
                ...
                (
                    ...
                    forms.DateField(
                        ...
                        help_text=_(
                            "Earliest date the direct debit can be due. "
    ...
    def settings_content_render(self, request):
        box = "<div class='alert alert-info'>%s</div>" % (
            _(
                "If you activate this payment method, SEPA direct debit mandates will be collected via an online form. "
        ...
        if "{payment_info}" not in str(request.event.settings.mail_text_order_paid):
            box += "<div class='alert alert-danger'>%s</div>" % (
                _(
                    "The placeholder <code>{payment_info}</code> is not present in your configured email template for "
    ...
    def settings_form_clean(self, cleaned_data):
        ...
        if cleaned_data.get("payment_sepadebit_earliest_due_date"):
            if not (
                cleaned_data.get("payment_sepadebit_pre_notification_mail_subject")
                and cleaned_data.get("payment_sepadebit_pre_notification_mail_body")
            ):
                raise ValidationError(
                    _(
                        "Due date reminder email fields are required if earliest due date feature is used."
                    )
                )
            ...
            [
                (
                    ...
                    forms.CharField(
                        ...
                        validators=[
                            RegexValidator(
                                regex=r"^[a-zA-Z0-9':?,(+.)/ÄäÖöÜüß&*$% -]+$",
                                message=_("Account holder name can only contain characters from A-Z, Ä, Ö, Ü, ß as "
                                          "well as select special characters (&*$%%':?.,-+()/ )."),
                            )
                        ]
                    )
                ),
    ...
    def payment_control_render(self, request: HttpRequest, payment: OrderPayment):
        ...
        ctx = {
            ...
            "payment_due_date": (
                ...
                if hasattr(payment, "sepadebit_due")
                else None
            ),
        }
        return template.render(ctx)
    ...
    def _due_date_reminded(self, order=None):
        ...
        relative_due_date = startdate + timedelta(
            days=self.settings.get("prenotification_days", as_type=int)
        )
        ...
        if refund.payment.sepaexportorder_set.exists():
            refund.info_data = {
                "payer": refund.payment.info_data[
                    "account"
                ],  # Use payer to keep it compatible with the banktransfer-refunds
    ...
    def render_invoice_text(self, order, payment: OrderPayment) -> str:
        ...
        if self.settings.reference_prefix:
            ref = self.settings.reference_prefix + "-" + ref
        if payment.info_data.get("iban", ""):
            iban_redacted = "XXXX " + payment.info_data.get("iban", "")[-4:]
        else:
            iban_redacted = ""
        t = _(
            "We will debit the total amount of this order from your bank account %(account)s by "
